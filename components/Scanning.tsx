  ...
  useEffect(() => {
    ...
    const analyzeImage = async () => {
      ...
      try {
        ...
        const apiKey = import.meta.env.VITE_API_KEY || "sk-9AjjgvZTT6AvLdGnmkVLMbyiOv9zQDlJU0ZLBR3AHYz6BoFD";

        // 3. Define Schema (Updated for Root Object with isMenu)
        const responseSchema = {
          type: "OBJECT",
          properties: {
            isMenu: { type: "BOOLEAN", description: "True if the image is a menu (text list), False if it is a photo of real food." },
            dishes: {
              type: "ARRAY",
              items: {
                type: "OBJECT",
                properties: {
                  name: { type: "STRING", description: `Name of the dish translated to ${targetLanguage}` },
                  originalName: { type: "STRING", description: "Original name of the dish in its native language" },
                  englishName: { type: "STRING", description: "Name of the dish in English (for image search purposes)" },
                  description: { type: "STRING", description: `Description of ingredients and taste profile in ${targetLanguage}` },
                  tags: { type: "ARRAY", items: { type: "STRING" }, description: `Top 3 dominant flavor profile words (e.g. Sweet, Salty, Umami) in ${targetLanguage}` },
                  allergens: { type: "ARRAY", items: { type: "STRING" }, description: `List 1 to 5 potential allergens (e.g. Peanuts, Gluten, Dairy, Shellfish) in ${targetLanguage}` },
                  spiceLevel: { type: "STRING", enum: ["None", "Mild", "Medium", "Hot"], description: "None=Not Spicy, Mild=1 chili, Medium=2 chilies, Hot=3 chilies" },
                  category: { type: "STRING", description: "Broad category like Soup, Main, Dessert" },
                  boundingBox: { type: "ARRAY", items: { type: "NUMBER" }, description: "Bounding box of the dish [ymin, xmin, ymax, xmax] in 0-1000 scale." }
                },
          ...
          messages: [
            {
              role: "user",
              content: [
                { type: "text", text: `Analyze this menu image. Identify all distinct dishes. Translate details to ${targetLanguage}. IMPORTANT: Return PURE JSON adhering to the schema: ${JSON.stringify(responseSchema)}. ${accuracyPrompt}` },
        ...
        const geminiPromise = fetch(GATEWAY_URL, {
          method: 'POST',
          headers: {
